...
exports.Matchers = {
     ...
     * `$2` target file
     */
    COPY_HEADER_MATCHER: /^CpHeader\s(.*\.h)\s(.*\.h)/m,
    /**
     * @regex Captured groups
     * `$1` source file
     * `$2` target file
     */
    COPY_PLIST_MATCHER: /^CopyPlistFile\s(.*\.plist)\s(.*\.plist)/m,
    /**
     * @regex Captured groups
     * `$1` file
     */
    COPY_STRINGS_MATCHER: /^CopyStringsFile.*\/(.*.strings)\s?[^(]+(?:\(in\s.*target '([^']*)'.*project '([^']*)'\)$)?/m,
    /**
     * @regex Captured groups
     * `$1` resource
     */
    CPRESOURCE_MATCHER: /^CpResource\s(.*)\s\//m,
     ...
     * `$1` = library
     */
    LIBTOOL_MATCHER: /^Libtool.*\/(.*\.a)\s?[^(]+(?:\(in\s.*target '([^']*)'.*project '([^']*)'\)$)?/m,
    /**
     * @regex Captured groups
     * `$1` = targetName
    ...
    PHASE_SUCCESS_MATCHER: /^\*\*\s(.*)\sSUCCEEDED\s\*\*/m,
    /**
     * @regex Captured groups
     * `$1` = script_name
     */
     ...
     * `$1` = project
     */
    PROCESS_PCH_MATCHER: /^ProcessPCH(?:\s|\+\+).*\s(.*.pch)\s?[^(]+(?:\(in\s.*target '([^']*)'.*project '([^']*)'\)$)?/m,
    /**
     * @regex Captured groups
     * `$1` filePath
     */
