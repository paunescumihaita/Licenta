...
function _default(program) {
  ...
  program.command('client:install:ios').description('Install Expo Go for iOS on the simulator').option('--latest', `Install the latest version of Expo Go, ignoring the current project version.`).helpGroup('client').asyncAction(async command => {
  ...
  program.command('client:install:android').description('Install Expo Go for Android on a connected device or emulator').option('--latest', `Install the latest version of Expo Go, ignore the current project version.`).helpGroup('client').asyncAction(async command => {
    const forceLatest = !!command.latest;
    const currentSdkConfig = await ClientUpgradeUtils().getExpoSdkConfig(process.cwd());
    const currentSdkVersion = currentSdkConfig ? currentSdkConfig.sdkVersion : undefined;
    const sdkVersions = await _xdl().Versions.sdkVersionsAsync();
