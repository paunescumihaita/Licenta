...
async function resolvePortAsync(projectRoot, defaultPort) {
  const port = defaultPort !== null && defaultPort !== void 0 ? defaultPort : _getenv().default.int('RCT_METRO_PORT', 8081); // Only check the port when the bundler is running.

  const resolvedPort = await (0, _choosePortAsync().choosePortAsync)(projectRoot, port);
